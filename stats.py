...
#     return df['Month'].value_counts(normalize=True)
...
def fetchbusyuser(df):
    ...
    newdf = pd.DataFrame(df['User'].value_counts(normalize=True) * 100)
    newdf = newdf.reset_index()
    newdf.columns = ['User', 'Percentage']
    return count, newdf
...
def getcommonwords(selecteduser, df):
    ...
    try:
        ...
        stopwords = ['a', 'the', 'in', 'to', 'of', 'and', 'is', 'it', 'for']  # Default stopwords
...
def monthtimeline(selecteduser, df):
    ...
    temp = df.groupby(['Year', 'Month_num', 'Month']).agg({'Message': 'count'}).reset_index()
...
def weekactivitymap(selecteduser, df):
    ...
    return df['Day_name'].value_counts(normalize=True)
